...
Geocodifica via Nominatim (OSM), baixa tiles Web Mercator e desenha o raio
com PyMuPDF. O PNG fica num store em memória por telefone — não vai no
contexto do modelo.
"""
...
def _render_map_png(
    ...
    label = format_radius(radius_m)
    north_lat, north_lng = destination_point(lat, lng, radius_m, 0.0)
    nx, ny = latlng_to_world_pixels(north_lat, north_lng, zoom)
    label_point = pymupdf.Point(nx - origin_x - 18, ny - origin_y - 6)
    page.insert_text(label_point, label, fontsize=13, color=(0.10, 0.20, 0.45))
...
async def build_radius_map(
    ...
    """Geocodifica (se preciso), desenha o mapa e guarda o PNG para o telefone."""
    try:
